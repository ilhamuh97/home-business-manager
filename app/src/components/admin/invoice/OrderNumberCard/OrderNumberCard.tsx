...
const RevenueCard = (props: IProps) => {
  ...
  const calculateMonthlyData = useCallback(() => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const filteredInvoices = filteredData.filter((order) => {
        const invoiceDate = dayjs(order.orderDate);
        return (
          invoiceDate.isSame(currentDate, "month") &&
          invoiceDate.isSame(currentDate, "year")
        );
      });

      results.push(filteredInvoices.length);
